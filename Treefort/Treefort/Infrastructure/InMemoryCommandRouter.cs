...
namespace Treefort.Infrastructure
{
    public class InMemoryCommandRouter : ICommandRouter
    {
        ...
        private readonly IDictionary<Type, IApplicationService> _routes = new Dictionary<Type, IApplicationService>();
        ...
        public void RegisterHandler(IApplicationService applicationService)
        {
            ...
            infos.ForEach(methodInfo => 
                _routes.Add(methodInfo.GetParameters().First().ParameterType, applicationService));
        }

        public IApplicationService GetServiceFor<T>(T command) where T : ICommand
        {
